...
public class sesion_controller extends HttpServlet {
 private PrintWriter respuesta;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //System.out.println("nameservlet:"+getServletName());
            ...
            switch (opcion) {
                ...
                case 2:
                    try {
                       ...
                       // System.out.println("boolean"+func.actualizarclave(identificacion));
                        if(func.actualizarclave(identificacion)){     
                            request.setAttribute("mensaje",new Gson().toJson("CLAVE ACTUALIZADA"));
                            request.getRequestDispatcher("login.jsp").forward(request, response);
                            }else{
                            request.setAttribute("mensaje",new Gson().toJson("CLAVE FALLO AL ACTUALIZAR"));
                         request.getRequestDispatcher("login.jsp").forward(request, response);
                        } 
                    } catch (Exception e) {
                       request.getRequestDispatcher("index.jsp").forward(request, response);
